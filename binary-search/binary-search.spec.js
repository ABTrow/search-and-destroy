...
const {binarySearch, extraBinarySearch} = require('./binary-search');
...
describe('Binary Search', () => {
	...
	describe('Extra Binary Search', () => {
		it('returns true if target value is in array', () =>{
			expect(extraBinarySearch(test1, 10)).to.equal(true);
			expect(extraBinarySearch(test2, -3)).to.equal(true);
			expect(extraBinarySearch(test2, 20)).to.equal(true);
			expect(extraBinarySearch(test3, 2.6)).to.equal(true);
		});

		it('returns false if target value is not in array', () => {
			expect(extraBinarySearch(test2, 25)).to.equal(false);
			expect(extraBinarySearch(test3, 4)).to.equal(false);
		});
